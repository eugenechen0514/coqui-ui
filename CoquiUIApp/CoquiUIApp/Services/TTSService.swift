...
actor TTSService {
    ...
    func synthesizeWithVoiceClone(text: String, referenceAudioPath: String, language: String) async throws -> Data {
        ...
        if let audioData = FileManager.default.contents(atPath: referenceAudioPath) {
            let originalFilename = URL(fileURLWithPath: referenceAudioPath).lastPathComponent
            body.append("--\(boundary)\r\n".data(using: .utf8)!)
            body.append("Content-Disposition: form-data; name=\"speaker_wav\"; filename=\"\(originalFilename)\"\r\n".data(using: .utf8)!)
