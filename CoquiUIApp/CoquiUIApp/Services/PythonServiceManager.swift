...
class PythonServiceManager: ObservableObject {
    ...
    func startServer(model: String? = nil) async throws {
        ...
        let scriptPath = serverScriptPath.path
        logs.append("Target server script: \(scriptPath)")
...
extension PythonServiceManager {
    ...
    static func checkTTSInstalled(pythonPath: String) async -> Bool {
        ...
        process.arguments = ["-c", "import sys; sys.path.append('.'); import TTS; print(TTS.__version__)"]
        ...
        do {
            ...
            if let output = String(data: data, encoding: .utf8) {
                print("Check TTS Output: \(output)")
            }
            
            return process.terminationStatus == 0
        } catch {
            print("Check TTS Error: \(error)")
            return false
        }
    }
}
